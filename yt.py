...
class Ui_MainWindow(object):
    ...
    def loadImage(self):
        ...
        """
        try:
            faceCascade = cv2.CascadeClassifier('datasets\cars1.xml')
        ...
        if cam:
            vid = cv2.VideoCapture('video\cctv.mp4')
        else:
            vid = cv2.VideoCapture('video\cctv.mp4')
        ...
        while(vid.isOpened()):
            ...
            try:
                ...
                for (x, y, w, h) in faces:
                    ...
                    plate_regions = reader.readtext(car_image, detail=0, paragraph=False, decoder='beamsearch', beamWidth=10)
                    for plate in plate_regions:
						# Track the position of the car
                        car_id = str(x) + str(y) + str(w) + str(h)
                        if car_id not in car_positions:
                            car_positions[car_id] = (x, y, time.time())
                        else:
                            prev_x, prev_y, prev_time = car_positions[car_id]
                            speed = abs(x - prev_x) / (time.time() - prev_time)
                            car_speeds[car_id] = speed
                            car_positions[car_id] = (x, y, time.time())

						# Draw a rectangle around the car and display the speed
                        speed_text = str(round(car_speeds.get(car_id, 0), 2)) + ' km/h'
                        cv2.rectangle(image, (x, y), (x+w, y+h), (0, 255, 0), 2)
                        cv2.putText(image, plate + ' ' + speed_text, (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)

						# Capture an image of the car if the car is already detected
                        if car_id in car_positions and car_id not in car_speeds:
                            car_filename = 'captures/' + plate + '.jpg'
                            cv2.imwrite(car_filename, car_image)
